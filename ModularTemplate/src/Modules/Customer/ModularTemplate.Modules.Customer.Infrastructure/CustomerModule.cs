...
using ProcessInboxJob = ModularTemplate.Modules.Customer.Infrastructure.Inbox.ProcessInboxJob;
using ProcessOutboxJob = ModularTemplate.Modules.Customer.Infrastructure.Outbox.ProcessOutboxJob;

namespace ModularTemplate.Modules.Customer.Infrastructure;
...
public static class CustomerModule
{
        ...
        IHostEnvironment environment)
    {
        ...
        // Outbox pattern
        services.Configure<OutboxOptions>(configuration.GetSection("Customer:Outbox"));
        services.ConfigureOptions<ConfigureProcessOutboxJob<ProcessOutboxJob>>();

        // Inbox pattern
        services.Configure<InboxOptions>(configuration.GetSection("Customer:Inbox"));
