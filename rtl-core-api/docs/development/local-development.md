...
dotnet run --project src/API/Rtl.Core.Api.Orders

# Run Sales module only
dotnet run --project src/API/Rtl.Core.Api.Sales
...
| Module | Command | URL |
|--------|---------|-----|
| Orders | `dotnet run --project src/API/Rtl.Core.Api.Orders` | `http://localhost:5001` |
| Sales | `dotnet run --project src/API/Rtl.Core.Api.Sales` | `http://localhost:5002` |
| Customer | `dotnet run --project src/API/Rtl.Core.Api.Customer` | `http://localhost:5003` |
| Organization | `dotnet run --project src/API/Rtl.Core.Api.Organization` | `http://localhost:5005` |
| Sample | `dotnet run --project src/API/Rtl.Core.Api.Sample` | `http://localhost:5006` |
...
{
  "ConnectionStrings": {
    "Database": "Host=localhost;Database=Rtl.Core_dev;Username=postgres;Password=postgres"
  }
}
```
...
dotnet ef migrations add InitialCreate \
  --project src/Modules/Orders/Rtl.Module.Orders.Infrastructure \
...
dotnet user-secrets init --project src/API/Rtl.Core.Api

# Set secrets
dotnet user-secrets set "ConnectionStrings:Database" "Host=localhost;..." --project src/API/Rtl.Core.Api
```
  ...
  "configurations": [
    {
      ...
      "type": "coreclr",
      "request": "launch",
      "program": "${workspaceFolder}/src/API/Rtl.Core.Api/bin/Debug/net9.0/Rtl.Core.Api.dll",
      "cwd": "${workspaceFolder}/src/API/Rtl.Core.Api"
    },
    {
      "name": "Launch Orders Module",
      "type": "coreclr",
      "request": "launch",
      "program": "${workspaceFolder}/src/API/Rtl.Core.Api.Orders/bin/Debug/net9.0/Rtl.Core.Api.Orders.dll",
      "cwd": "${workspaceFolder}/src/API/Rtl.Core.Api.Orders"
    }
  ]
}
```
...
```bash
dotnet watch run --project src/API/Rtl.Core.Api
```
...
```bash
# Unit tests
dotnet test src/Modules/Orders/test/Rtl.Module.Orders.Domain.Tests
...
```bash
dotnet watch test --project src/Modules/Orders/test/Rtl.Module.Orders.Domain.Tests
```
